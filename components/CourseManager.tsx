            ...
            <div className="mt-8 flex items-center justify-between pt-6 border-t dark:border-slate-800">
               ...
               {editingCourse?.id === c.id && (
                 <div className="flex items-center space-x-4">
                   <button onClick={() => setShowMaterialEditor({ isVisible: true, type: 'lecturer_shared' })} className="text-[10px] font-black text-emerald-600 uppercase">+ Add Material</button>
                   <label className={`flex items-center space-x-2 text-[10px] font-black text-brand-600 uppercase cursor-pointer ${isUploading ? 'opacity-50 cursor-wait' : ''}`}>
                     <Icons.Plus />
