...
const ArchiveViewer: React.FC<ArchiveViewerProps> = ({ archives, onRestore }) => {
  return (
    <div className="h-full space-y-8 overflow-y-auto custom-scrollbar pb-20">
      <header>
        <h3 className="text-xl font-black text-slate-800 dark:text-slate-100 uppercase tracking-tighter" dir="rtl">סשנים מארכיב</h3>
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1" dir="rtl">היסטוריית תמונות מצב ומדדי ביצועים</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div key={archive.id} className="bg-white dark:bg-slate-850 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="text-right" dir="rtl">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">ממוצע כיתה</p>
            ...
            <div className="grid grid-cols-3 gap-4 mt-8 pt-6 border-t dark:border-slate-800">
              <div className="text-center">
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">מצוין</p>
                <p className="text-emerald-500 font-black">{archive.stats?.distribution?.high || 0}</p>
              </div>
              <div className="text-center">
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">עבר</p>
                <p className="text-brand-500 font-black">{archive.stats?.distribution?.mid || 0}</p>
              </div>
              <div className="text-center">
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">קריטי</p>
        ...
        {archives.length === 0 && (
          <div className="col-span-full py-20 text-center border-2 border-dashed dark:border-slate-800 rounded-[3rem]">
            ...
            <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest" dir="rtl">אין ארכיבים שמורים</p>
          </div>
          </div>
        )}
      </div>
