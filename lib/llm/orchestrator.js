...
export class LLMOrchestrator {
  constructor() {
    ...
    const allProviders = order
      .filter(name => PROVIDER_MAP[name])
      .map(name => new PROVIDER_MAP[name]());
    // Only keep providers that have valid API keys configured
    this.providers = allProviders.filter(p => !p._unavailable);
    const available = this.providers.map(p => p.name);
    const skipped = allProviders.filter(p => p._unavailable).map(p => p.name);
    if (available.length > 0) {
      console.log(`[LLMOrchestrator] Available providers: ${available.join(' → ')}`);
    }
    if (skipped.length > 0) {
      console.warn(`[LLMOrchestrator] Skipped (no API key): ${skipped.join(', ')}`);
    }
    if (this.providers.length === 0) {
      console.error('[LLMOrchestrator] No LLM providers configured! Set at least one API key (GEMINI_API_KEY, GROQ_API_KEY, or OPENAI_API_KEY).');
    }
    this._log = [];
  }
  ...
  async evaluateWithFallback(prompt, options = {}) {
    if (this.providers.length === 0) {
      throw new Error('No LLM providers configured. Set at least one API key: GEMINI_API_KEY, GROQ_API_KEY, or OPENAI_API_KEY.');
    }
