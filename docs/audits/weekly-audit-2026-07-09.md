...
|------|-------|-------------|
| `.claude/settings.local.json` | 26 | MongoDB Atlas URI — user `Vercel-Admin-st-system-db`, password `<REDACTED>` |
| `.claude/settings.local.json` | 27 | MongoDB Atlas URI — user `Vercel-Admin-st-system-db`, password `<REDACTED>` |

#### Evidence

```json
// .claude/settings.local.json:26 (git-tracked, committed in history)
"Bash(MONGODB_URI=\"mongodb://Vercel-Admin-st-system-db:<REDACTED>@ac-d5h6cow-shard-00-00.lznnim2.mongodb.net:...\" node:*)",
