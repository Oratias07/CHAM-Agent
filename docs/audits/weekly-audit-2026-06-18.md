...
Two distinct passwords (`AFGkvWViMkf9ucHT` and `RsPJMRrw5lAVWBl5`) for the Atlas user `Vercel-Admin-st-system-db` are embedded inline in Claude Code permission allowlist entries. The file is tracked in git (confirmed by `git ls-files .claude/settings.local.json`) and is **not** listed in `.gitignore`.
...
1. **[IMMEDIATE]** Rotate the two exposed MongoDB Atlas passwords (`AFGkvWViMkf9ucHT`, `RsPJMRrw5lAVWBl5`) — treat as compromised. Add `.claude/settings.local.json` to `.gitignore` and scrub git history.
